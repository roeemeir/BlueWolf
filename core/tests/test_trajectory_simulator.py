...
def test_double_opening_priority_sweep_is_qa_only() -> None:
    assert DOUBLE_HIPPODROME_PRIORITY_OPENINGS_DEG == (
        10.0,
        15.0,
        20.0,
        25.0,
    ...
    assert full[:7] == DOUBLE_HIPPODROME_PRIORITY_OPENINGS_DEG
    assert any(angle < 10.0 for angle in full)
    assert any(angle > 40.0 for angle in full)


def test_double_opening_is_not_programmatically_limited_to_priority_range() -> None:
    for angle in (5.0, 25.0, 55.0):
        ...
        assert route.shape is RouteShape.SO_DOUBLE_HIPPODROME
        assert route.metadata["opening_deg"] == angle
        assert route.xy_m.shape == (512, 2)
        assert np.all(np.isfinite(route.xy_m))


def test_simulator_is_seed_deterministic_with_wind_noise_and_turn_loss() -> None:
    route = make_route(
...
def test_turn_biased_loss_can_remove_turn_samples_without_inventing_points() -> None:
    ...
    assert np.count_nonzero(~observed_route) > 0
    assert np.count_nonzero(observed_route) > 0
    # Missing data stays missing: observed coordinates still exist only as simulator
    # truth/noise state, while observed_mask is the authoritative network visibility.
    assert trace.observed_xy_m.shape == trace.truth_xy_m.shape
