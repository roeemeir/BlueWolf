...
## מחקר 17 — הפרדת ראיות מפה לפי שרת וזמן

22/09/2026. בסיס הבדיקה: `7191b5b2ae144f82a860f08571072cfb93a23f24`.

ב־Web נמצאו שני מנגנוני ערבוב: סמן היסטורי שלא מצא frame החזיר null לזמן חיתוך העקבה, אך null בפונקציית החיתוך פירושו LIVE; בנוסף, ראיית recompute נשמרה לפי event/template בלי זהות שרת. אותם מזהי אירוע וקבוצה בשרתים שונים יכלו להציג גאומטריה משרת קודם.

התיקון משמר serverId ו־groupId בתוך LiveMapEventEvidence, שולח serverId בבקשה, ומקבל תשובה רק בהתאמת שרת/קבוצה/אירוע/תבנית. guard בעת הרינדור חוסם cache קודם גם לפני שה־effect החדש מסיים; cleanup מבטל שימוש בתשובה מאוחרת. override רטרואקטיבי מותר רק לשרת/קבוצה/אירוע הנבחרים, אך רשאי להשתמש בתבנית החדשה שבחר המפעיל. כאשר cursor היסטורי קיים ואין frame תואם, העקבה המוצגת ריקה; בחזרה ל־LIVE נשמרת העקבה הנצפית. אין שינוי לזיהוי, קיבוץ, ציון, ארכיון או מקור נתוני ה־Core.

`tests/operational-map-provenance.test.mjs` מפעיל את הרכיב עצמו עם runtime ו־hooks מבוקרים. ארבע רגרסיות נכשלו לפני התיקון; לאחריו עברו כל שש הבדיקות, לרבות תשובה שמגיעה אחרי מעבר שרת ותשובה המכילה serverId שגוי. אלה ראיות חישוב ורינדור SVG, לא אימות חזותי של דפדפן או מכשיר. נותר לבדוק חורי ניווט שנמחקים לפני איסוף העקבות ואת כלל צירופי השכבות.

## מחקר 18 — חלון ארכיון ושעון הבדיקה

בדיקת report API השוותה ארכיון נע בן שבעה ימי UTC לטווח תאריכים קבוע. ב־22/09 פגו חלק מימי התרחיש ולכן מספר האירועים ירד, אף שה־retention פעל כנדרש. הבדיקה מקפיאה Date ל־19/09/2026 בלבד בסביבת Node test; לא נוסף פרמטר לקוח שמשנה את שעון השרת. היא דורשת 30 אירועים בדיוק ו־7 תאריכים היסטוריים שונים. בדיקה נוספת מקדמת את השעון מחצות פחות מילישנייה לחצות ומוודאת שארבעת אירועי היום הישן נעלמים, ושה־API מחזיר 422 ללא fallback ל־Core. אין שינוי של חלון השמירה או מודל SIM.
