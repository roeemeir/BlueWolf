...
def detect_closed_route(
    ...
    effective_points = tuple(
        item.point for item, keep in zip(raw_points, inlier_mask, strict=True) if keep
    )
...
def _fit_shape(
    ...
    *,
    ...
    xy = statistics.fmean(
        (point.x_m - center.x_m) * (point.y_m - center.y_m) for point in points
    )
    orientation = 0.5 * math.atan2(2.0 * xy, xx - yy)

    cos_o = math.cos(orientation)
    sin_o = math.sin(orientation)
    projected_long = tuple(
        (point.x_m - center.x_m) * cos_o + (point.y_m - center.y_m) * sin_o
        for point in points
    )
    projected_short = tuple(
        -(point.x_m - center.x_m) * sin_o + (point.y_m - center.y_m) * cos_o
        for point in points
    )
    ...
    canonical = resample_closed_polyline(
        canonical,
        min(config.canonical_point_limit, 64),
    )
    return _Shape(
        center=center,
...
def _stadium_points(
    ...
    for index in range(1, straight_points + 1):
        ...
        output.append(
            _rotate_from_local(
                center,
                half_straight * (1.0 - 2.0 * fraction),
                radius,
                orientation_rad,
            )
        )
...
def _rotate_from_local(
    ...
    return CanonicalPoint(
        center.x_m + along * cos_o - across * sin_o,
        center.y_m + along * sin_o + across * cos_o,
    )
...
def _transient_bump_mask(points: Sequence[CanonicalPoint]) -> tuple[bool, ...]:
    ...
    for index in range(1, len(points) - 1):
        ...
        if (
            incoming > threshold
            and outgoing > threshold
            and bridge <= max(2.5 * median_step, 1.5)
        ):
            mask[index] = False
    return tuple(mask)
...
def _fit_fraction(
    ...
    if not points:
        return 0.0
    fitted = sum(
        project_onto_closed_polyline(canonical, point).distance_m <= tolerance_m
        for point in points
    )
...
def _has_closed_cycle(
    ...
    *,
    ...
    accumulated = 0.0
    threshold = max(
        short_axis_m * config.closure_distance_short_axis_ratio,
        1.0,
    )
...
def _estimate_period_seconds(
    ...
    *,
    ...
    velocity_speeds = tuple(
        math.hypot(
            float(item.sample.velocity_east_mps),
            float(item.sample.velocity_north_mps),
        )
