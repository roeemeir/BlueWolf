...
def _representative_cycle(samples: Sequence[Mapping[str, Any]], fit: _base._Pca) -> list[Mapping[str, Any]]:
    ...
    The initial estimate intentionally uses single-hippodrome length.  A
    Figure-8 is exactly that route class with crossed legs, so the estimate is a
    good first window and avoids counting repeated traversals as artificial
    self-intersections.
    """
    if len(samples) <= 24:
        return list(samples)
    ...
    start = end - __import__("datetime").timedelta(seconds=max(30.0, rough_period * 1.08))
...
def _canonical_figure8_points(samples: Sequence[Mapping[str, Any]], fit: _base._Pca) -> tuple[list[dict[str, float]], int]:
    ...
    leaves that turn through the positive minor-axis half.  This makes phase
...
def _segments(geometry: Mapping[str, Any]) -> list[dict[str, Any]]:
    # Figure-8 has the same two external turns/axis as a single hippodrome.  The
    # legs cross internally, which must not make it look like a double entity
    # for inter-route grouping.
...
# Install the refined primitive once for the stable envelope.  All public
# analysis/history calls below still execute the same application_analysis
# pipeline, now with the topology-aware track builder.
