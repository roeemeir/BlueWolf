...
function estimateTemplateScores(
  group: DemoGroup,
  active: SyncTemplate | undefined,
  preview: SyncTemplate | undefined,
  thresholds: ReturnType<typeof useWorkspace>["state"]["thresholds"],
) {
  ...
  if (group.family === "SI") {
    ...
    position = errors.length
      ? errors.reduce((sum, value) => sum + errorScore(value, thresholds.siPositionFullDeg, thresholds.siPositionZeroDeg), 0) / errors.length
      : 100;
  } else {
...
function GroupCard({ group, vehicleTypes, selected, templateName, onSelect, onVehicle, onTemplate }: {
  group: DemoGroup;
  ...
  return <article className={`v09-group-card glass-panel ${selected ? "active" : ""}`}>
    <button className="v09-group-select" onClick={onSelect} type="button">
      <div><span className="v09-group-dot" style={{ background: color }} /><strong>{group.name}</strong><small>{group.subtitle}</small></div>
      <ScoreRing value={group.total} color={color} />
    </button>
    ...
    <div className="v09-member-list">{group.members.map((member) => {
      const type = vehicleTypes.find((item) => item.id === member.typeId);
      return <button type="button" key={member.id} onClick={() => onVehicle(member.id)}><TypeGlyph type={type}/><span><b>רכב {member.id}</b><small>{type?.name}</small></span><strong className={tone(member.score)}>{member.score}</strong></button>;
    })}</div>
  </article>;
}

function TemplateSheet({ group, activeId, templates, vehicleTypes, thresholds, onClose, onApply }: {
  group: DemoGroup;
  activeId: string;
  ...
  thresholds: ReturnType<typeof useWorkspace>["state"]["thresholds"];
  onClose: () => void;
  onApply: (templateId: string, mode: "now" | "event-start") => void;
}) {
  ...
  const preview = candidates.find((item) => item.id === previewId) ?? active ?? candidates[0];
  const expected = estimateTemplateScores(group, active, preview, thresholds);
  const entities = preview?.soSpec?.entities?.map((entity, index) => ({
    id: `sheet-${index}`,
    kind: entity.kind,
    vehicleTypeId: entity.vehicleTypes[0] ?? vehicleTypes[index % Math.max(1, vehicleTypes.length)]?.id ?? "storm",
    vehicleCount: Math.max(1, entity.vehicleTypes.length),
  })) ?? [];
  ...
  return <div className="v09-template-sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="v09-template-sheet" role="dialog" aria-modal="true" aria-label="החלפת תבנית">
      <header><div><p className="eyebrow">Template what-if</p><h2>החלפת תבנית · {group.name}</h2><p>הציון הצפוי משתמש בחוק הספים והמשקולות של הליבה. שינוי זווית מעבר לסף מוריד את Position עד 0.</p></div><button type="button" className="v09-icon-btn" onClick={onClose}><X/></button></header>
...
export function OperatorViewV09({ serverId, serverName, dataMode, onDataModeChange, onInvestigate }: {
  serverId: string;
  serverName: string;
  dataMode: DataMode;
  ...
  const muted = mutedUntil === Number.MAX_SAFE_INTEGER || mutedUntil > nowMs;
  ...
  const applyTemplate = async (id: string, mode: "now" | "event-start") => {
    const next = {
      ...state,
    ...
    toast.success(mode === "event-start" ? "התבנית הוחלה מתחילת האירוע וחישוב התחקור עודכן" : "התבנית הוחלה מעכשיו");
  };
  const muteFor = (minutes: number | "restart") => {
    setMutedUntil(minutes === "restart" ? Number.MAX_SAFE_INTEGER : Date.now() + minutes * 60_000);
    toast.success(minutes === "restart" ? "מושתק עד הפעלה מחדש" : `מושתק ל-${minutes} דקות`);
  };
  ...
  return <div className="v09-operator">
    ...
    <div className="v09-operator-grid">
      <section className="glass-panel v09-map-panel" ref={mapRef}>
        ...
        <div className="v09-map-layers"><button className={showTrace ? "active" : ""} onClick={() => setShowTrace((value) => !value)}>עקבה · 5s</button><button className={showRoutes ? "active" : ""} onClick={() => setShowRoutes((value) => !value)}>נתיבים</button><button className={showGroups ? "active" : ""} onClick={() => setShowGroups((value) => !value)}>קבוצות</button><button className={showRelations ? "active" : ""} onClick={() => setShowRelations((value) => !value)}>תבנית</button><button className={showScoreTrace ? "active" : ""} onClick={() => setShowScoreTrace((value) => !value)}>עקבה לפי ציון</button><button className={showGrid ? "active" : ""} onClick={() => setShowGrid((value) => !value)}>Grid</button></div>
    ...
    <section className="glass-panel v09-timeline-panel"><div className="v09-panel-head"><div><h3>ציר ציונים</h3><p>שתי הקבוצות מוצגות יחד; הגבולות נחתכים לפי חלון הזמן.</p></div><div className="v09-window-row">{([30, 60, 90, 120] as const).map((value) => <button key={value} className={windowMinutes === value ? "active" : ""} onClick={() => { setWindowMinutes(value); setCursor(119); }}>{value} דק׳</button>)}</div></div><div className="v09-layer-legend">{(["total", "sync", "route"] as ScoreLayer[]).map((layer) => <button key={layer} className={layers.includes(layer) ? "active" : ""} onClick={() => toggleLayer(layer)}><i className={`style-${layer}`}/>{layer === "total" ? "כולל" : layer === "sync" ? "סנכרון" : "נתיב"}</button>)}</div><TimelineChartV09 serverId={serverId} windowMinutes={windowMinutes} layers={layers} cursor={cursor} onCursor={setCursor} selectedVehicle={selectedVehicle}/><div className="v09-playbar"><button className="v09-icon-btn" onClick={() => setRunning((value) => !value)}>{running ? <Pause/> : <Play/>}</button><input type="range" min={Math.max(0, 120 - windowMinutes)} max={119} value={clamp(cursor, Math.max(0, 120 - windowMinutes), 119)} onChange={(event) => { setRunning(false); setCursor(Number(event.target.value)); }}/><span>{cursor - (120 - windowMinutes) + 1}/{windowMinutes} דק׳</span></div></section>

    {templateSheet && <TemplateSheet group={selected} activeId={activeTemplateId} templates={state.templates} vehicleTypes={state.vehicleTypes} thresholds={state.thresholds} onClose={() => setTemplateSheet(false)} onApply={applyTemplate}/>} 
  </div>;
}
