...
function viewportProjector(rows: readonly GeoPoint[]) {
  ...
  const minX = Math.min(...local.map((row) => row.x)); const maxX = Math.max(...local.map((row) => row.x));
  const minY = Math.min(...local.map((row) => row.y)); const maxY = Math.max(...local.map((row) => row.y));
  const centerX = (minX + maxX) / 2; const centerY = (minY + maxY) / 2;
  const rawSpanX = Math.max(maxX - minX, EPS); const rawSpanY = Math.max(maxY - minY, EPS);
  const spanX = Math.max(rawSpanX * 1.24, rawSpanY * 0.35, 1e-5); const spanY = Math.max(rawSpanY * 1.24, rawSpanX * 0.35, 1e-5);
  const scale = Math.min((VIEW_WIDTH - 2 * MARGIN_X) / spanX, (VIEW_HEIGHT - 2 * MARGIN_Y) / spanY);
  return (latitude: number, longitude: number) => ({ x: VIEW_WIDTH / 2 + (longitude * longitudeScale - centerX) * scale, y: VIEW_HEIGHT / 2 - (latitude - centerY) * scale });
}
...
export function OperationalLiveMap({
  ...
  const project = viewportProjector([...history.map((point) => ({ latitude: point.latitude, longitude: point.longitude })), ...current, ...routePoints]);
  ...
  return <div className="operational-map-layer-shell" dir="rtl" data-requirements="OP-02 OP-04">
    ...
    <svg className="map-svg v04-live-map engineering" viewBox={`0 0 ${VIEW_WIDTH} ${VIEW_HEIGHT}`} role="img" aria-label="מפת מיקומים מבצעית של רכבי Blue Wolf">
      ...
      <rect width={VIEW_WIDTH} height={VIEW_HEIGHT} fill="var(--map-bg)" /><rect width={VIEW_WIDTH} height={VIEW_HEIGHT} className="v04-map-wash" />
...
export const operationalMapInternals = { viewportProjector };
