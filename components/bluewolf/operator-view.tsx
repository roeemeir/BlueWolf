...
export function OperatorView({ serverId, serverName, dataMode, onDataModeChange, onInvestigate }: { serverId: string; serverName: string; dataMode: DataMode; onDataModeChange: (mode: DataMode) => void; onInvestigate: () => void }) {
  ...
  useEffect(() => { const update = () => setNowMs(Date.now()); update(); const timer = window.setInterval(update, 1000); return () => window.clearInterval(timer); }, []);
  ...
  const events = dataMode === "influx" ? structuralEvents(serverId) : [];
  const muted = muteUntil === "restart" || (typeof muteUntil === "number" && muteUntil > nowMs);
  ...
  return <div className="operator-workspace v04-operator">
    ...
    <div className="v04-source-switch"><span>מקור נתונים</span><button type="button" className={dataMode === "simulation" ? "active" : ""} onClick={() => onDataModeChange("simulation")}>סימולציה</button><button type="button" className={dataMode === "influx" ? "active" : ""} onClick={() => onDataModeChange("influx")}>Influx</button></div>
  </div>;
}
