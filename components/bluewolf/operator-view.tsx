...
export function OperatorView({ serverId, serverName, dataMode, onDataModeChange, onInvestigate }: { serverId: string; serverName: string; dataMode: DataMode; onDataModeChange: (mode: DataMode) => void; onInvestigate: () => void }) {
  ...
  return <div className="operator-workspace v04-operator">
    ...
    {activeAlert && <section className={`active-alert v04-live-alert glass-panel ${activeAlert.severity}`}><TriangleAlert /><div><span>התראה חיה · {activeAlertGroup?.id}</span><strong>{activeAlert.title}</strong><p>{activeAlert.detail}</p></div><div className="alert-actions"><Button variant="outline" size="sm" onClick={() => setMuted((v) => !v)}>{muted ? <VolumeX /> : <Volume2 />}{muted ? "מושתק" : "השתק"}</Button><Button size="sm" onClick={() => toast.success("ההתראה סומנה כטופלה; היא לא הופכת לאירוע תחקור")}><BellRing />טופל</Button></div></section>}
