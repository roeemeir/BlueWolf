...
import { ScoreRing, TimelineChart, VehicleIconGlyph, type GroupKey, type ScoreLayer } from "./visuals";
...
export function OperatorView({ serverId, serverName, dataMode, onDataModeChange, onInvestigate }: { serverId: string; serverName: string; dataMode: DataMode; onDataModeChange: (mode: DataMode) => void; onInvestigate: () => void }) {
  ...
  useEffect(() => {
    ...
    oscillator.frequency.value = activeAlert.severity === "critical" ? 1046.5 : 880;
  ...
  const muteFor = (value: 5 | 15 | 30 | "restart") => {
    setMutedUntil(value === "restart" ? "restart" : Date.now() + value * 60_000);
    toast.success(value === "restart" ? "התראות קוליות הושתקו עד הפעלה מחדש" : `התראות קוליות הושתקו ל־${value} דקות`);
  };
