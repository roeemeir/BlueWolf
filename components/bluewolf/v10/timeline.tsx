...
import { navigationHistory } from "./nav-engine";
...
const STYLE: Record<TimelineLayer, { width: number; dash?: string; label: string }> = {
  total: { width: 4.2, label: "כולל" }, sync: { width: 2.5, dash: "10 6", label: "סנכרון" }, route: { width: 2.3, dash: "2 7", label: "נתיב" },
};

export function V10Timeline({ serverId, currentTick, windowMinutes, layers, groups, cursor, onCursor, windMode = "off", thresholds, weights, siTemplate, soTemplate, groupingSettings }: {
  serverId: string; currentTick: number; windowMinutes: 30 | 60 | 90 | 120; layers: TimelineLayer[]; groups: V10GroupKey[]; cursor: number; onCursor: (value: number) => void;
  windMode?: WindMode; thresholds: ScoreThresholds; weights: ScoreWeights; siTemplate?: SyncTemplate; soTemplate?: SyncTemplate; groupingSettings: SoGroupingSettings;
}) {
  const visible = navigationHistory({ serverId, currentTick, minutes: windowMinutes, windMode, thresholds, weights, siTemplate, soTemplate, groupingSettings });
  const left = 56, right = 1040, top = 24, bottom = 246;
  ...
  return <div className="v09-timeline-wrap">
    <svg className="v09-timeline" viewBox="0 0 1100 330" role="img" aria-label={`ציונים מחושבים מנתוני ניווט ב-${windowMinutes} הדקות האחרונות`} onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); const ratio = Math.max(0, Math.min(1, (event.clientX - rect.left) / Math.max(1, rect.width))); onCursor(Math.round(ratio * (visible.length - 1))); }}>
      ...
      {timeTicks.map((ago) => { const index = Math.round((windowMinutes - ago) / windowMinutes * (visible.length - 1)); return <g key={ago}><line x1={x(index)} x2={x(index)} y1={bottom} y2={bottom+5} className="v09-chart-tick" /><text x={x(index)} y="270" textAnchor="middle" className="v09-chart-label">לפני {ago} דק׳</text></g>; })}
    ...
    <div className="v09-timeline-caption"><span>הציון מחושב מאותה סדרת ניווט שמזינה את המפה.</span><span>הרוח אינה רכיב בציון; אם היא מזיזה את הניווט, ההשפעה נוצרת דרך המדידה בלבד.</span></div>
  </div>;
}
