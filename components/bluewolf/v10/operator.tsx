...
export function OperatorViewV10({ serverId, serverName, dataMode, onInvestigate }: { serverId: string; serverName: string; dataMode: DataMode; onInvestigate: () => void }) {
  ...
  const [windMode, setWindMode] = useState<WindMode>(dataMode === "simulation" ? "gusty" : "off");
  ...
  useEffect(() => { if (dataMode !== "simulation") setWindMode("off"); else if (windMode === "off") setWindMode("gusty"); }, [dataMode]);
  ...
  const scores = { si: scoreFor("si", siTemplate, serverId, tick, state, windMode), so: scoreFor("so", soTemplate, serverId, tick, state, windMode) };
  ...
  const expected = preview ? scoreFor(selectedGroup, preview, serverId, tick, state, windMode) : currentScore;
