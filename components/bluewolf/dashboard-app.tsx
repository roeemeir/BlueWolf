  ...
  return (
    <main className="app-shell">
      <header className="topbar glass-panel">
        ...
        <Dialog><DialogTrigger asChild><button type="button" className="live-state"><span className="live-dot" /><div><strong>חי · סימולציה</strong><small>{storageMode === "cloud" ? "שמירה מרכזית פעילה" : "שמירה מקומית"}</small></div></button></DialogTrigger><DialogContent className="glass-dialog" dir="rtl"><DialogHeader><DialogTitle>מצב המערכת</DialogTitle><DialogDescription>סביבת ההדגמה מפעילה את כל זרימות הממשק על נתוני סימולציה. חיבור Influx יופעל בפריסה המבצעית.</DialogDescription></DialogHeader><div className="system-dialog-grid"><span><Radio />מקור נתונים<b>סימולטור דטרמיניסטי</b></span><span><HardDrive />אחסון<b>{storageMode === "cloud" ? "D1 מרכזי" : "Local fallback"}</b></span><span><CheckCircle2 />ליבה<b>43/43 בדיקות</b></span><span><Database />קונפיגורציה<b>גרסה {revision || 1}</b></span></div></DialogContent></Dialog>
        <div className="top-actions">
          <Select value={serverValue} onValueChange={setServer}><SelectTrigger className="server-select"><Database /><SelectValue /></SelectTrigger><SelectContent>{state.servers.filter((item) => item.enabled).map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select>
      ...
      <Tabs value={tab} onValueChange={(value) => setTab(value as MainTab)} dir="rtl" className="main-tabs">
        <div className="nav-row glass-panel"><TabsList variant="line"><TabsTrigger value="operator"><Activity />מבצעי</TabsTrigger><TabsTrigger value="investigation"><FileChartColumn />תחקור לאחור</TabsTrigger><TabsTrigger value="developer"><Settings2 />מפתחים</TabsTrigger></TabsList><div className="nav-status"><span><Wifi />{activeServer}</span><span><CheckCircle2 />ליבה תקינה</span><Badge variant="outline">v0.2</Badge></div></div>
        <TabsContent value="operator"><OperatorView serverName={activeServer} onInvestigate={() => setTab("investigation")} /></TabsContent>
