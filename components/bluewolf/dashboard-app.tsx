...
function AppInner() {
  ...
  // A valid score computed by Python Core from synthetic GPS is still TEST,
  // even if the source selector says Influx and all downstream contracts pass.
  const testNavigationCore = dataMode === "influx" && coreSnapshot?.serverId === serverValue && coreSnapshot.source.kind === "python-core" && coreSnapshot.source.navigationOrigin === "simulation" && coreSnapshot.source.syntheticNavigation === true;
  const runtimeLabel = dataMode === "simulation" ? "סימולציה" : testNavigationCore ? "TEST NAVIGATION · Python Core · לא מבצעי" : operationalEvidenceReady ? "Python Core · נתונים מאומתים" : runtimeState === "healthy" ? "Core · נתונים חלקיים" : runtimeState === "stale" ? "Core מיושן" : runtimeState === "connecting" ? "מתחבר ל-Core" : "Core לא זמין";
  ...
  return <main className="app-shell v04-shell">
    <header className="topbar glass-panel">
      ...
      <Dialog>
        <DialogTrigger asChild><button type="button" className={`live-state source-${dataMode}`}><span className="live-dot" /><div><strong>{dataMode === "simulation" ? "SIM · סימולציה" : testNavigationCore ? `TEST · ${runtimeLabel}` : operationalEvidenceReady ? `חי · ${runtimeLabel}` : `לא מאומת · ${runtimeLabel}`}</strong><small>{runtimeDetail}</small></div></button></DialogTrigger>
        <DialogContent className="glass-dialog source-dialog" dir="rtl"><DialogHeader><DialogTitle>מקור הנתונים</DialogTitle><DialogDescription>בחירת מקור אינה משנה שרת או זירה. נתוני ניווט סינתטיים המוזנים לליבה נשארים מסומנים TEST גם כאשר Python Core מחשב מהם נתיבים וציונים תקפים.</DialogDescription></DialogHeader><div className="source-choice-grid"><button type="button" className={dataMode === "simulation" ? "active" : ""} onClick={() => changeDataMode("simulation")}><Radio /><strong>סימולציה</strong><span>תרחיש דטרמיניסטי</span></button><button type="button" className={dataMode === "influx" ? "active" : ""} onClick={() => changeDataMode("influx")}><Database /><strong>{testNavigationCore ? "TEST NAVIGATION + Python Core" : "InfluxDB 2 + Python Core"}</strong><span>{dataMode === "influx" ? runtimeLabel : "runtime מבצעי"}</span></button></div><div className="system-dialog-grid"><span><HardDrive />אחסון<b>{storageMode === "cloud" ? "מרכזי" : "מקומי"}</b></span><span><CheckCircle2 />קונפיגורציה<b>גרסה {revision || 1}</b></span><span><Clock3 />טיק<b>{state.settings.uiRefreshSeconds} שניות</b></span></div></DialogContent>
      </Dialog>
      <div className="top-actions">
        ...
        <Sheet>
          <SheetTrigger asChild><Button variant="outline" size="icon" aria-label={`התראות ${dataMode === "simulation" ? "סימולציה" : testNavigationCore ? "Core TEST" : "Core"}, ${unread} לא נקראו`}><Bell />{unread > 0 && <span className="notification-count">{unread}</span>}</Button></SheetTrigger>
          <SheetContent side="left" className="notification-sheet glass-sheet" dir="rtl">
            <SheetHeader><SheetTitle>התראות {dataMode === "simulation" ? "סימולציה · SIM" : testNavigationCore ? "TEST NAVIGATION · Python Core" : "Python Core"}</SheetTitle><SheetDescription>{dataMode === "simulation" ? "התרעות תרחיש הדמיה בלבד — לא נתונים מבצעיים." : testNavigationCore ? "התראות שחושבו בליבה מנתוני ניווט סינתטיים לצורכי TEST בלבד — לא נתונים מבצעיים." : "התראות פעילות מה־snapshot התקף האחרון של השרת הנבחר. התראות אינן אירועי תחקור."}</SheetDescription></SheetHeader>
            <div className="notification-list">
              ...
              {notifications.map((item) => <button type="button" key={item.key} className={`notification-item ${readAlertKeys.includes(item.key) ? "read" : ""}`} onClick={() => setReadAlertKeys((current) => current.includes(item.key) ? current : [...current, item.key])}>
                ...
                <div><strong>{testNavigationCore ? "TEST · " : ""}{item.sourceLabel} · {item.title}</strong><p>{item.detail}</p><small>שרת {item.serverId} · קבוצה {item.groupId} · {notificationTime(item.activeSince, state.settings.timezone)}</small></div>
              </button>)}
            </div>
          </SheetContent>
        </Sheet>
    ...
    <Tabs value={tab} onValueChange={(value) => setTab(value as MainTab)} dir="rtl" className="main-tabs">
      <div className="nav-row glass-panel"><TabsList variant="line"><TabsTrigger value="operator"><Activity />מבצעי</TabsTrigger><TabsTrigger value="investigation"><FileChartColumn />תחקור</TabsTrigger><TabsTrigger value="developer"><Settings2 />מפתחים</TabsTrigger></TabsList><div className="nav-status"><span><Wifi />{activeServer}</span><span className={`mode-chip ${dataMode}`}>{dataMode === "simulation" ? "SIM" : testNavigationCore ? "TEST CORE" : operationalEvidenceReady ? "CORE" : "NO CORE SCORE"}</span><Badge variant="outline">v0.4 QA</Badge></div></div>
