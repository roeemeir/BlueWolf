...
function AppInner() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      setRuntimeState((current) => current === "healthy" || current === "stale" ? current : "connecting");
      try {
    ...
    void poll();
    timer = window.setInterval(() => void poll(), Math.max(1, state.settings.uiRefreshSeconds) * 1000);
