...
import { Activity, Beaker, Bell, CheckCircle2, Clock3, Database, FileChartColumn, HardDrive, Info, Moon, Radio, Settings2, Sun, TriangleAlert, Wifi } from "lucide-react";
...
import { appendLivePositionHistory } from "@/lib/live-position-history";
...
type MainTab = "operator" | "investigation" | "developer" | "calibration";
...
const INITIAL_NOTIFICATIONS: NotificationItem[] = [
  { id: 1, tone: "warning", title: "SO-02 · התראה חיה", detail: "איחור בפנייה — התראה אינה אירוע מבני", time: "עכשיו", read: false },
  { id: 2, tone: "info", title: "נתיב זוהה", detail: "הנתיב האפקטיבי עודכן", time: "לפני 4 דק׳", read: false },
  { id: 3, tone: "success", title: "QA", detail: "בדיקות הליבה האחרונות עברו", time: "לפני 12 דק׳", read: true },
];
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    const bootstrapHistory = async () => {
      try {
        ...
        // Timeline history is not a liveness dependency. Latest continues to drive the operator.
      }
    };
    ...
    const poll = async () => {
      try {
        ...
        appendLivePositionHistory(snapshot);
  ...
  const runtimeLabel = dataMode === "simulation"
    ? "סימולציה"
  ...
  return <main className="app-shell v04-shell">
    <header className="topbar glass-panel">
      <button type="button" className="brand" onClick={() => setTab("operator")}>
        <div className="brand-mark"><WolfLogo /></div>
        <div><h1>זאב כחול</h1><p>ניטור סנכרון רכבים</p></div>
      </button>

      <Dialog>
        <DialogTrigger asChild><button type="button" className={`live-state source-${dataMode}`}><span className="live-dot" /><div><strong>חי · {runtimeLabel}</strong><small>{runtimeDetail}</small></div></button></DialogTrigger>
        <DialogContent className="glass-dialog source-dialog" dir="rtl">
          <DialogHeader><DialogTitle>מקור הנתונים</DialogTitle><DialogDescription>בחירת מקור אינה משנה שרת או זירה. במצב Influx הציונים מוצגים רק כאשר Python Core מחזיר snapshot תקף.</DialogDescription></DialogHeader>
          <div className="source-choice-grid">
            <button type="button" className={dataMode === "simulation" ? "active" : ""} onClick={() => changeDataMode("simulation")}><Radio /><strong>סימולציה</strong><span>תרחיש דטרמיניסטי</span></button>
          ...
          <div className="system-dialog-grid"><span><HardDrive />אחסון<b>{storageMode === "cloud" ? "מרכזי" : "מקומי"}</b></span><span><CheckCircle2 />קונפיגורציה<b>גרסה {revision || 1}</b></span><span><Clock3 />טיק<b>{state.settings.uiRefreshSeconds} שניות</b></span></div>
        </DialogContent>
      </Dialog>
      ...
      <div className="top-actions">
        ...
        <Sheet>
          <SheetTrigger asChild><Button variant="outline" size="icon"><Bell />{unread > 0 && <span className="notification-count">{unread}</span>}</Button></SheetTrigger>
          <SheetContent side="left" className="notification-sheet glass-sheet" dir="rtl"><SheetHeader><SheetTitle>התראות חיות</SheetTitle><SheetDescription>Alerts הם ישות נפרדת מ־Structural Events.</SheetDescription></SheetHeader><div className="notification-list">{notifications.map((item) => <button type="button" key={item.id} className={`notification-item ${item.read ? "read" : ""}`} onClick={() => setNotifications((current) => current.map((entry) => entry.id === item.id ? { ...entry, read: true } : entry))}><span className={`notification-icon ${item.tone}`}>{item.tone === "warning" ? <TriangleAlert /> : item.tone === "success" ? <CheckCircle2 /> : <Info />}</span><div><strong>{item.title}</strong><p>{item.detail}</p><small>{item.time}</small></div></button>)}</div></SheetContent>
        </Sheet>
      </div>
    </header>

    <Tabs value={tab} onValueChange={(value) => setTab(value as MainTab)} dir="rtl" className="main-tabs">
      <div className="nav-row glass-panel">
        <TabsList variant="line">
          <TabsTrigger value="operator"><Activity />מבצעי</TabsTrigger>
          <TabsTrigger value="investigation"><FileChartColumn />תחקור</TabsTrigger>
          <TabsTrigger value="developer"><Settings2 />מפתחים</TabsTrigger>
          <TabsTrigger value="calibration"><Beaker />כיול</TabsTrigger>
        </TabsList>
        <div className="nav-status"><span><Wifi />{activeServer}</span><span className={`mode-chip ${dataMode}`}>{dataMode === "simulation" ? "SIM" : runtimeState === "healthy" ? "CORE" : "NO CORE"}</span><Badge variant="outline">SRS v1.1</Badge></div>
      </div>
