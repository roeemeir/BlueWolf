...
import { InvestigationReportPanel } from "./investigation-report-panel";
...
function AppInner() {
  const { state, ready, loadProgress, storageMode, revision } = useWorkspace(); const { resolvedTheme, setTheme } = useTheme(); const [clock, setClock] = useState("--:--:--"); const [server, setServer] = useState("1"); const [dataMode, setDataMode] = useState<DataMode>("simulation"); const [tab, setTab] = useState<MainTab>("operator"); const [runtimeState, setRuntimeState] = useState<RuntimeUiState>("simulation"); const [runtimeDetail, setRuntimeDetail] = useState("תרחיש דטרמיניסטי"); const [, setRuntimeRevision] = useState(0); const [notifications, setNotifications] = useState([{ id: 1, tone: "warning", title: "SO-02 · התראה חיה", detail: "איחור בפנייה — לא נוצר אירוע תחקור חדש", time: "עכשיו", read: false }, { id: 2, tone: "info", title: "נתיב זוהה", detail: "הנתיב האפקטיבי עודכן", time: "לפני 4 דק׳", read: false }]);
