    ...
    <div style={{ display: "grid", gridTemplateColumns: "minmax(180px,.7fr) minmax(0,2fr)", gap: 14, marginTop: 12 }}>
      ...
      {draft ? <div style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div><Label>Token mode</Label><Select value={draft.tokenMode} onValueChange={(value) => patch("tokenMode", value as MapTokenMode)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">ללא token</SelectItem><SelectItem value="bearer">Bearer header</SelectItem><SelectItem value="query">Query parameter</SelectItem><SelectItem value="path">Path placeholder</SelectItem></SelectContent></Select></div>
          {draft.tokenMode === "query" && <div><Label>Query parameter</Label><Input value={draft.tokenQueryParam ?? "token"} onChange={(event) => patch("tokenQueryParam", event.target.value)} dir="ltr" /></div>}
          {draft.tokenMode === "path" && <div><Label>Path placeholder</Label><Input value={draft.tokenPathPlaceholder ?? "__BLUEWOLF_MAP_TOKEN__"} onChange={(event) => patch("tokenPathPlaceholder", event.target.value)} placeholder="__BLUEWOLF_MAP_TOKEN__" dir="ltr" /><p className="card-hint">ה־URL חייב להכיל placeholder זה במקום ה־API key. הערך הסודי נשמר בנפרד ומוזרק רק בבקשה לשרת.</p></div>}
        </div>
