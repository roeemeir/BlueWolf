  ...
  return <section className="glass-panel" dir="rtl" data-requirements="BW-OFF-010 BW-OFF-012" style={{ padding: 16, marginBottom: 16 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "minmax(180px,.7fr) minmax(0,2fr)", gap: 14, marginTop: 12 }}>
      ...
      {draft ? <div style={{ display: "grid", gap: 10 }}>
        ...
        {draft.kind === "wms" && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><div><Label>CRS</Label><Select value={draft.crs ?? "CRS:84"} onValueChange={(value) => patch("crs", value)}><SelectTrigger dir="ltr"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="CRS:84">CRS:84</SelectItem><SelectItem value="EPSG:4326">EPSG:4326</SelectItem></SelectContent></Select></div><div><Label>Version</Label><Input value={draft.version ?? "1.3.0"} onChange={(event) => patch("version", event.target.value)} dir="ltr" /></div></div>}
