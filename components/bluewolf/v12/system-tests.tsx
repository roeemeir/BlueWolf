...
import { DEFAULT_SO_GROUPING, soPairCompatibility } from "../v10/grouping";
...
import { buildAnalysisHistory, deriveEvents } from "./navigation-history";
...
export function V12SystemTests() {
  ...
  const run = async (stress = 180) => {
    ...
    try {
      const now = new Date();
      add("Core contract", "Architecture", CORE_API_VERSION === "1.0.0", `CORE_API_VERSION=${CORE_API_VERSION}`);
      ...
      for (const serverId of ["1", "2", "3"]) {
        ...
        const analysis = analyzeNavigationDataset(dataset, options);
        const gt = simulatorGroundTruthAt(serverId, center, grouping);
        analysisCount += 1;

        const detected = Object.keys(analysis.current).map(Number);
        add(`שרת ${serverId} · רכבים פעילים`, "Simulator→NAV→Core→GT", gt.activeVehicles.every((id) => detected.includes(id)), `GT ${ids(gt.activeVehicles)} · ניתוח ${ids(detected)}`);
        add(`שרת ${serverId} · טווח ציונים`, "Core scoring", [analysis.groups.si.score, analysis.groups.so.score].every((score) => Object.values(score).every((value) => Number.isFinite(value) && value >= 0 && value <= 100)), `SI ${analysis.groups.si.score.total} · SO ${analysis.groups.so.score.total}`);

        if (serverId === "2") add("שרת 2 · נתיב SO לא חוקי נשאר מחוץ לקבוצה", "Core grouping", gt.ungroupedVehicles.every((id) => analysis.ungroupedVehicles.includes(id)), `GT מחוץ ${ids(gt.ungroupedVehicles)} · בפועל ${ids(analysis.ungroupedVehicles)}`);
        if (serverId === "3") add("שרת 3 · Double+Single מזוהים כקבוצת SO", "Core grouping", analysis.groups.so.members.length >= 2, `SO בפועל ${ids(analysis.groups.so.members)}`);
      ...
      add("חוק קיבוץ · valid/invalid", "Core grouping", valid.valid && !invalid.valid, `valid=${valid.explanation} | invalid=${invalid.explanation}`);
      ...
      const history = buildAnalysisHistory(sixHours, options, 80, 12);
      const events = deriveEvents(history, state.thresholds);
      analysisCount += history.length;
      add("אירועים · נגזרים מהיסטוריית NAV", "Investigation", events.length > 0 && events.every((event) => event.frames.length > 0 && event.startReason.length > 10 && event.endReason.length > 10), `${events.length} אירועים מתוך ${history.length} frames`);
      ...
      const empty = analyzeNavigationDataset({
        samples: [],
      ...
      for (let index = 0; index < stress; index += 1) {
        ...
        const analysis = analyzeNavigationDataset(data, options);
        analysisCount += 1;
      ...
      add(`${stress} חלונות stress אמיתיים`, "Stress", !stressFailure, stressFailure || `${stress} datasets עברו simulator→raw NAV→core`);
    } catch (error) {
