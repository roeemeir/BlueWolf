...
export async function loadNavigationDataset({ mode, serverId, from, to, grouping, windMode, influx, targetPoints = 9_000 }: {
  ...
  try {
    ...
    return {
      dataset: { samples, provenance: provenanceFromSamples("influx", serverId, from, to, samples, warnings) },
      error: samples.length ? null : "Influx לא החזיר דגימות ניווט מלאות בטווח המבוקש.",
    };
  } catch (error) {
