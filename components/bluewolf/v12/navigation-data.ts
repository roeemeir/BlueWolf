...
export type RawNavigationSample = {
  ...
  velocityEast: number;
  /** Local metric coordinates used by the analysis/rendering engine. 1 unit = 1 metre. */
  x: number;
  y: number;
};
...
export type SimulatorGroundTruth = {
  ...
  routeKinds: Record<number, "circle" | "single" | "double">;
  routeKeys: Record<number, string>;
};
...
export function simulationHistoryBounds(now = new Date()) {
  return { from: new Date(now.getTime() - SIMULATION_HISTORY_DAYS * 86_400_000), to: now };
}
...
function localToGeo(x: number, y: number) {
  return {
    latitude: SIM_ORIGIN_LAT - (y - 285) / METRES_PER_DEG_LAT,
    longitude: SIM_ORIGIN_LON + (x - 500) / METRES_PER_DEG_LON,
  };
}

function snapshotLocal(serverId: string, timestamp: Date, grouping: SoGroupingSettings, windMode: WindMode) {
  const tick = simulationTickAt(timestamp);
  const scenario = getV09Scenario(serverId, tick, grouping);
  const members = [...scenario.groups.si.members, ...scenario.groups.so.members, ...(scenario.ungroupedMembers ?? [])];
  const points = new Map<number, { x: number; y: number; heading: number; routeKey: string; kind: "circle" | "single" | "double" }>();
  for (const member of members) {
    const route = scenario.routes.find((item) => item.key === member.routeKey);
    if (!route) continue;
    const ideal = pointOnClosed(route.points, member.phase);
    const noise = deterministicNoise(serverId, tick, member.id);
    const wind = windOffsetPx(serverId, tick, member.id, windMode);
    points.set(member.id, { x: ideal.x + noise.x + wind.x, y: ideal.y + noise.y + wind.y, heading: ideal.heading, routeKey: route.key, kind: route.kind });
  }
...
function sampleAt(serverId: string, timestamp: Date, grouping: SoGroupingSettings, windMode: WindMode) {
  const current = snapshotLocal(serverId, timestamp, grouping, windMode);
  const previousTime = new Date(timestamp.getTime() - 2_000);
  const previous = snapshotLocal(serverId, previousTime, grouping, windMode);
  const out: RawNavigationSample[] = [];
  for (const [vehicleId, point] of current.points) {
    const prev = previous.points.get(vehicleId) ?? point;
    // x grows east, y grows south. Convert local displacement to N/E m/s.
    const velocityEast = (point.x - prev.x) / 2;
    const velocityNorth = -(point.y - prev.y) / 2;
    const geo = localToGeo(point.x, point.y);
    out.push({
      source: "simulation",
      serverId,
...
export function simulatorGroundTruthAt(serverId: string, timestamp: Date, grouping: SoGroupingSettings): SimulatorGroundTruth {
  const scenario = getV09Scenario(serverId, simulationTickAt(timestamp), grouping);
  const routeKinds: SimulatorGroundTruth["routeKinds"] = {};
  const routeKeys: Record<number, string> = {};
  const all = [...scenario.groups.si.members, ...scenario.groups.so.members, ...(scenario.ungroupedMembers ?? [])];
  for (const member of all) {
    const route = scenario.routes.find((item) => item.key === member.routeKey);
    if (route) routeKinds[member.id] = route.kind;
    routeKeys[member.id] = member.routeKey;
  }
  return {
    timestamp: timestamp.toISOString(), serverId,
    activeVehicles: all.map((item) => item.id).sort((a, b) => a - b),
    siVehicles: scenario.groups.si.members.map((item) => item.id).sort((a, b) => a - b),
    soVehicles: scenario.groups.so.members.map((item) => item.id).sort((a, b) => a - b),
    ungroupedVehicles: (scenario.ungroupedMembers ?? []).map((item) => item.id).sort((a, b) => a - b),
    routeKinds, routeKeys,
  };
}
...
export function provenanceFromSamples(source: NavigationSource, serverId: string, from: Date, to: Date, samples: RawNavigationSample[], warnings: string[] = []): NavigationProvenance {
  ...
  const times = [...new Set(samples.map((sample) => new Date(sample.timestamp).getTime()))].sort((a, b) => a - b);
  const gaps = times.slice(1).map((value, index) => (value - times[index]) / 1000).filter((value) => value > 0);
  const med = median(gaps);
  const latestMs = times.at(-1) ?? null;
  const expected = med && med > 0 ? Math.max(1, Math.round((to.getTime() - from.getTime()) / 1000 / med) + 1) * Math.max(1, vehicles.size) : null;
  return {
    source, serverId, from: from.toISOString(), to: to.toISOString(), latestSampleAt: latestMs == null ? null : new Date(latestMs).toISOString(),
    sampleCount: samples.length, vehicleCount: vehicles.size, samplingMedianSeconds: med,
    completenessPct: expected ? Math.min(100, samples.length / expected * 100) : null,
    freshnessSeconds: latestMs == null ? null : Math.max(0, (to.getTime() - latestMs) / 1000), warnings,
  };
}
...
export function generateSimulationDataset({ serverId, from, to, grouping, windMode = "gusty", targetPoints = 9_000 }: { serverId: string; from: Date; to: Date; grouping: SoGroupingSettings; windMode?: WindMode; targetPoints?: number }): NavigationDataset {
  const bounds = simulationHistoryBounds(to > new Date() ? to : new Date());
  ...
  // Keep short windows detailed and long historical queries bounded/reproducible.
  const stepSeconds = Math.max(2, Math.ceil(durationSec * 8 / Math.max(1, targetPoints)));
...
export function simulationFixtureDataset(serverId: string, center: Date, grouping: SoGroupingSettings, windMode: WindMode = "gusty", minutes = 12) {
  return generateSimulationDataset({ serverId, from: new Date(center.getTime() - minutes * 60_000), to: center, grouping, windMode, targetPoints: 3_500 });
}
