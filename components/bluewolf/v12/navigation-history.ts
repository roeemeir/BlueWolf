...
import { analyzeNavigationDataset, compareMembership, type NavigationDerivedAnalysis } from "./navigation-analyzer";

export type AnalysisOptions = { thresholds: ScoreThresholds; weights: ScoreWeights; siTemplate?: SyncTemplate; soTemplate?: SyncTemplate; groupingSettings: SoGroupingSettings };
export type AnalysisFrame = { timestamp: string; analysis: NavigationDerivedAnalysis };
export type DerivedEvent = {
  id: string; index: number; start: string; end: string; family: "SI" | "SO"; groupKey: "si" | "so"; members: number[]; startReason: string; endReason: string;
  startEvidence: string[]; endEvidence: string[]; frames: AnalysisFrame[]; representative: NavigationDerivedAnalysis;
};
...
export function buildAnalysisHistory(dataset: NavigationDataset, options: AnalysisOptions, maxFrames = 61, lookbackMinutes = 12): AnalysisFrame[] {
  ...
  return selected.map((timestamp) => {
    ...
    return { timestamp, analysis: analyzeNavigationDataset(slice, options) };
  });
}

function routeSignature(analysis: NavigationDerivedAnalysis, key: "si" | "so") {
...
function boundaryReason(previous: AnalysisFrame | null, current: AnalysisFrame, key: "si" | "so", thresholds: ScoreThresholds) {
  const group = current.analysis.groups[key]; if (!previous) return { text: `תחילת טווח: זוהתה קבוצת ${key.toUpperCase()} עם ${group.members.length} רכבים מתוך נתוני הניווט.`, evidence: [`רכבים: ${group.members.join(", ") || "אין"}`, `Sync ${group.score.sync}`, `Route ${group.score.route}`] };
  const membership = compareMembership(previous.analysis, current.analysis)[key]; const membershipReason = membershipText(membership); if (membershipReason) return { text: `שינוי חברות שאושר בחלון הניתוח: ${membershipReason}.`, evidence: [`לפני: ${previous.analysis.groups[key].members.join(", ") || "אין"}`, `אחרי: ${group.members.join(", ") || "אין"}`] };
  const oldRoute = routeSignature(previous.analysis, key), newRoute = routeSignature(current.analysis, key); if (oldRoute !== newRoute) return { text: "זוהה שינוי במשפחת/גאומטריית הנתיב מתוך עקבות הניווט ולכן נפתח אירוע חדש.", evidence: [`לפני: ${oldRoute || "אין"}`, `אחרי: ${newRoute || "אין"}`] };
  const beforePeriod = previous.analysis.groups[key].periodErrorPct, nowPeriod = group.periodErrorPct; if ((beforePeriod <= thresholds.periodFullPct && nowPeriod > thresholds.periodFullPct) || (beforePeriod < thresholds.periodZeroPct && nowPeriod >= thresholds.periodZeroPct)) return { text: `פער זמן המחזור חצה סף: ${beforePeriod.toFixed(1)}% → ${nowPeriod.toFixed(1)}%.`, evidence: [`סף מלא ${thresholds.periodFullPct}%`, `סף אפס ${thresholds.periodZeroPct}%`] };
...
export function deriveEvents(history: AnalysisFrame[], thresholds: ScoreThresholds): DerivedEvent[] {
  ...
  for (const key of ["si", "so"] as const) {
    ...
    for (let i = 0; i < history.length; i++) {
      const frame = history[i]; const group = frame.analysis.groups[key]; if (!group.members.length) { if (currentFrames.length) flush("הקבוצה חדלה להתקיים/להיות ניתנת לזיהוי בנתוני הניווט.", ["אין חברי קבוצה מזוהים"]); currentState = ""; continue; }
