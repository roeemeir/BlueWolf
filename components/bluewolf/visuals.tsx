...
function capsulePoint(a: Point, b: Point, radius: number, phase: number) {
  const p = ((phase % 1) + 1) % 1;
  const u = unit(a, b); const n = normal(a, b); const len = distance(a, b); const straight = len; const turn = Math.PI * radius; const perimeter = straight * 2 + turn * 2; let s = p * perimeter;
  let point: Point; let tangent: Point;
...
function doublePoint(a: Point, middle: Point, b: Point, radius: number, phase: number) {
  const p = ((phase % 1) + 1) % 1;
  return p < .5 ? capsulePoint(a, middle, radius, p * 2) : capsulePoint(middle, b, radius, (p - .5) * 2);
}

const SO_ANCHORS = [{ x: 475, y: 362 }, { x: 585, y: 267 }, { x: 700, y: 230 }, { x: 815, y: 267 }, { x: 925, y: 362 }];
...
export function LiveMap({ serverId, tick, selectedGroup, selectedVehicle, showTrace, showRoutes, showRelations, showGrid, vehicleTypes, templateValues, mapProfile = "engineering", animate = true, onSelectGroup, onSelectVehicle }: { serverId: string; tick: number; selectedGroup: GroupKey; selectedVehicle: number | null; showTrace: boolean; showRoutes: boolean; showRelations: boolean; showGrid: boolean; vehicleTypes: VehicleType[]; templateValues?: Partial<Record<GroupKey, number[]>>; mapProfile?: string; animate?: boolean; onSelectGroup: (key: GroupKey) => void; onSelectVehicle: (id: number, group: GroupKey) => void }) {
  ...
  const soPoints = soMembers.map((vehicle, index) => {
    const phase = (progress + vehicle.phase) % 1;
    if (index === 0) return { ...capsulePoint(SO_ANCHORS[0], SO_ANCHORS[1], 30, phase), vehicle };
    if (index === soMembers.length - 1) return { ...capsulePoint(SO_ANCHORS[3], SO_ANCHORS[4], 30, phase), vehicle };
    return { ...doublePoint(SO_ANCHORS[1], SO_ANCHORS[2], SO_ANCHORS[3], 28, index === 1 ? phase : (phase + .5) % 1), vehicle };
  });
  ...
  return <svg className={`map-svg v04-live-map ${mapClass}`} viewBox="0 0 1000 570" role="img" aria-label="מפה חיה של קבוצות SI ו-SO">
    ...
    <g className="v04-map-labels"><text x="38" y="45">SI · טבעות</text><text x="455" y="45">SO · שרשרת היפודרומים</text><text x="455" y="68">קצה משותף · 30° בין שכנים</text></g>
    {showRoutes && <g className="v04-routes">
      <g className={selectedGroup === "si" ? "active" : ""} onClick={() => onSelectGroup("si")}>{(["inner", "middle", "outer"] as const).map((ring) => <circle key={ring} cx={siCenter.x} cy={siCenter.y} r={ringRadius[ring]} className="v04-si-route" />)}</g>
      <g className={selectedGroup === "so" ? "active" : ""} onClick={() => onSelectGroup("so")}><path d={capsulePath(SO_ANCHORS[0], SO_ANCHORS[1], 30)} className="v04-so-route" /><path d={capsulePath(SO_ANCHORS[1], SO_ANCHORS[2], 28)} className="v04-so-route double" /><path d={capsulePath(SO_ANCHORS[2], SO_ANCHORS[3], 28)} className="v04-so-route double" /><path d={capsulePath(SO_ANCHORS[3], SO_ANCHORS[4], 30)} className="v04-so-route" /><circle cx={SO_ANCHORS[1].x} cy={SO_ANCHORS[1].y} r="7" className="v04-shared-end" /><circle cx={SO_ANCHORS[3].x} cy={SO_ANCHORS[3].y} r="7" className="v04-shared-end" /></g>
    </g>}
    {showTrace && <g className="v04-traces"><circle cx={siCenter.x} cy={siCenter.y} r="83" /><path d={capsulePath({ x: 474, y: 365 }, { x: 587, y: 265 }, 32)} /><path d={capsulePath({ x: 814, y: 265 }, { x: 928, y: 365 }, 32)} /></g>}
    {showRelations && selectedGroup === "so" && <g><RelationBadge x={SO_ANCHORS[1].x} y={SO_ANCHORS[1].y} relation={relations[0] ?? "opposite"} side="left" /><RelationBadge x={SO_ANCHORS[3].x} y={SO_ANCHORS[3].y} relation={relations[1] ?? "same"} side="right" /></g>}
...
export function TimelineChart({ serverId = "1", selected, layers, cursor, onCursor, fromLabel = "17:00", toLabel = "19:00", selectedVehicle }: { serverId?: string; selected: GroupKey; layers: ScoreLayer[]; cursor: number; onCursor: (value: number) => void; fromLabel?: string; toLabel?: string; selectedVehicle?: number | null }) {
  ...
  return <svg className="timeline-svg v04-timeline" viewBox="0 0 1000 260" onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); const relative = (event.clientX - rect.left) / rect.width; onCursor(Math.round(Math.max(0, Math.min(1, relative)) * (series.length - 1))); }}>
    ...
    {layers.flatMap((layer) => (["si", "so"] as GroupKey[]).map((group) => <polyline key={`${group}-${layer}`} points={layerPoints(group, layer)} fill="none" stroke={groupLineColor[group]} strokeWidth={group === selected && layer === "sync" ? 3.5 : 2} strokeDasharray={layer === "route" ? "8 5" : layer === "total" ? "2 4" : undefined} opacity={group === selected ? .95 : .48} />))}
    ...
    <text x={left} y="254" className="chart-label">{fromLabel}</text><text x={right} y="254" textAnchor="end" className="chart-label">{toLabel}</text>
  </svg>;
}
...
export function TemplatePreview({ family, values, compact = false, title, vehicleTypes = [], soKinds = ["single", "double", "single"] }: { family: Family | GroupKey; values: number[]; compact?: boolean; title?: string; vehicleTypes?: VehicleType[]; soKinds?: SoRouteKind[] }) {
  ...
  const count = normalized === "SI" ? pairCountToVehicleCount(values.length) : 0;
  const typeColors = vehicleTypes.length ? vehicleTypes.map((item) => item.color) : ["#ff9f43", "#34b7eb", "#9068ff", "#d16ff2", "#4fbf79"];
  if (normalized === "SI") {
    const positions = Array.from({ length: count }, (_, index) => index === 0 ? 0 : values[firstPairIndex(index)] ?? 90);
    const points = positions.map((angle, index) => { const radius = [42, 64, 86, 64, 86][index]; const rad = (angle - 90) * Math.PI / 180; return { x: 200 + Math.cos(rad) * radius, y: 115 + Math.sin(rad) * radius, angle }; });
    let pairIndex = 0;
    return <svg className={`template-preview-svg v04-template-preview ${compact ? "compact" : ""}`} viewBox="0 0 400 230" role="img" aria-label={title ?? "תצוגת תבנית SI"}><rect width="400" height="230" rx="20" /><circle cx="200" cy="115" r="42" className="ring inner" /><circle cx="200" cy="115" r="64" className="ring middle" /><circle cx="200" cy="115" r="86" className="ring outer" />{points.map((point, index) => <g key={index}><circle cx={point.x} cy={point.y} r="9" fill={typeColors[index % typeColors.length]} /><text x={point.x} y={point.y - 14} textAnchor="middle">R{index + 1}</text></g>)}<g className="v04-preview-pairs">{points.flatMap((point, first) => points.slice(first + 1).map((other) => { const value = values[pairIndex++] ?? 90; const mid = lerp(point, other, .5); return <g key={`${first}-${other.x}`}><line x1={point.x} y1={point.y} x2={other.x} y2={other.y} /><rect x={mid.x - 23} y={mid.y - 11} width="46" height="22" rx="11" /><text x={mid.x} y={mid.y + 4} textAnchor="middle">{value}°</text></g>; }))}</g></svg>;
  }
  const relations = values.map(relationFromCode); const a = { x: 38, y: 155 }; const b = { x: 125, y: 82 }; const c = { x: 200, y: 56 }; const d = { x: 275, y: 82 }; const e = { x: 362, y: 155 };
  return <svg className={`template-preview-svg v04-template-preview ${compact ? "compact" : ""}`} viewBox="0 0 400 220" role="img" aria-label={title ?? "תצוגת תבנית SO"}><rect width="400" height="220" rx="20" /><g className="v04-preview-so"><path d={capsulePath(a, b, 21)} /><path d={capsulePath(b, c, 19)} className="double" /><path d={capsulePath(c, d, 19)} className="double" /><path d={capsulePath(d, e, 21)} /><circle cx={b.x} cy={b.y} r="5" /><circle cx={d.x} cy={d.y} r="5" /><g className="v04-preview-relation"><rect x="77" y="25" width="96" height="25" rx="12" /><text x="125" y="42" textAnchor="middle">{SO_RELATION_LABELS[relations[0] ?? "opposite"]} · 30°</text><rect x="227" y="25" width="96" height="25" rx="12" /><text x="275" y="42" textAnchor="middle">{SO_RELATION_LABELS[relations[1] ?? "same"]} · 30°</text></g>{[82, 165, 235, 318].map((x, index) => <circle key={x} cx={x} cy={[132, 88, 88, 132][index]} r="7" fill={typeColors[index % typeColors.length]} />)}<text x="200" y="205" textAnchor="middle">{soKinds.map((kind) => kind === "double" ? "כפול" : "יחיד").join(" — ")}</text></g></svg>;
}

export function EventMiniMap({ family, color }: { family: GroupKey; color: string }) { return <div className="event-mini-map v04-event-mini"><MapPinned /><span>{family.toUpperCase()}</span>{family === "si" ? <div className="mini-circle-route" style={{ borderColor: color }} /> : <svg viewBox="0 0 80 36"><path d={capsulePath({ x: 8, y: 26 }, { x: 34, y: 10 }, 5)} stroke={color} fill="none" /><path d={capsulePath({ x: 34, y: 10 }, { x: 55, y: 10 }, 5)} stroke={color} fill="none" /><path d={capsulePath({ x: 55, y: 10 }, { x: 76, y: 26 }, 5)} stroke={color} fill="none" /></svg>}</div>; }

export function EventOverviewMap({ eventLabels = ["E1", "E2", "E3"] }: { eventLabels?: string[] }) { return <svg className="v04-overview-map" viewBox="0 0 720 300"><rect width="720" height="300" rx="22" /><g className="v04-overview-routes"><circle cx="150" cy="150" r="75" /><circle cx="150" cy="150" r="49" /><path d={capsulePath({ x: 330, y: 205 }, { x: 420, y: 120 }, 22)} /><path d={capsulePath({ x: 420, y: 120 }, { x: 515, y: 105 }, 20)} /><path d={capsulePath({ x: 515, y: 105 }, { x: 610, y: 205 }, 22)} /></g>{eventLabels.map((label, index) => <g className="v04-overview-event" key={label} transform={`translate(${[150, 435, 560][index % 3]} ${[150, 145, 185][index % 3]})`}><circle r="17" /><text y="5" textAnchor="middle">{label}</text></g>)}</svg>; }
...
export function RouteBankMap({ routes, vehicleTypes, selectedId, onSelect, onMove }: { routes: SavedRoute[]; vehicleTypes: VehicleType[]; selectedId: string | null; onSelect: (id: string) => void; onMove: (id: string, x: number, y: number) => void }) {
  const svgRef = useRef<SVGSVGElement>(null); const [dragId, setDragId] = useState<string | null>(null);
  const pointer = (event: React.PointerEvent<SVGSVGElement>) => { const rect = svgRef.current?.getBoundingClientRect(); if (!rect) return { x: 50, y: 50 }; return { x: Math.max(5, Math.min(95, (event.clientX - rect.left) / rect.width * 100)), y: Math.max(8, Math.min(92, (event.clientY - rect.top) / rect.height * 100)) }; };
