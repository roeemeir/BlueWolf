...
function Timeline({ result }: { result: EventRecomputeResult }) {
  const values = result.points.map((point) => point.group.total).filter((value): value is number => value !== null);
  ...
  return <div style={{ marginTop: 14 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 6 }}><strong>ציון כולל לאורך האירוע</strong><span>{result.points.length} frames מה־Core</span></div><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Recomputed event score timeline" style={{ width: "100%", minHeight: 180 }}>{[0, 25, 50, 75, 100].map((score) => { const y = height - score / 100 * height; return <g key={score}><line x1="0" x2={width} y1={y} y2={y} stroke="currentColor" opacity=".12" /><text x="4" y={Math.max(12, y - 4)} fontSize="18" fill="currentColor" opacity=".6">{score}</text></g>; })}<path d={path} fill="none" stroke="currentColor" strokeWidth="5" vectorEffect="non-scaling-stroke" /></svg></div>;
}
...
function RecomputeResultView({ state }: { state: Extract<RecomputeState, { kind: "complete" }> }) {
  ...
  return <div style={{ marginTop: 18 }}><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Badge variant={state.persisted ? "default" : "outline"}>{state.persisted ? <CheckCircle2 /> : <AlertTriangle />}{state.persisted ? "נשמר" : "לא נשמר ב-Workspace"}</Badge><Badge variant="outline">run {result.runId}</Badge><Badge variant="outline">template {result.templateId}</Badge><Badge variant="outline">tpl-ver {result.templateVersion.slice(0, 14)}</Badge><Badge variant="outline">code {result.codeVersion.slice(0, 10)}</Badge><Badge variant="outline">config {result.configVersion.slice(0, 10)}</Badge></div><div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 10, marginTop: 14 }}><div className="metric-card"><span>Sync ממוצע</span><strong>{scoreLabel(result.summary.sync)}</strong></div><div className="metric-card"><span>Route ממוצע</span><strong>{scoreLabel(result.summary.route)}</strong></div><div className="metric-card"><span>Total ממוצע</span><strong>{scoreLabel(result.summary.total)}</strong></div></div><Timeline result={result} /><div style={{ display: "grid", gridTemplateColumns: "minmax(220px, .8fr) minmax(0, 1.2fr)", gap: 14, marginTop: 16 }}><div><strong>Root causes</strong><div style={{ display: "grid", gap: 6, marginTop: 8 }}>{result.rootCauses.length ? result.rootCauses.map((cause) => <div key={cause.reason} className="glass-panel" style={{ padding: 10, display: "flex", justifyContent: "space-between" }}><span>{cause.reason}</span><b>{cause.occurrences}</b></div>) : <span>אין primary reasons בריצה זו.</span>}</div></div><div><strong>מצב אחרון לפי רכב</strong><div style={{ display: "grid", gap: 6, marginTop: 8 }}>{last?.members.map((member) => <div key={member.memberId} className="glass-panel" style={{ padding: 10, display: "grid", gridTemplateColumns: "1fr auto auto auto", gap: 10 }}><span>{member.memberId} · {member.slotId}</span><b>S {scoreLabel(member.sync)}</b><b>R {scoreLabel(member.route)}</b><b>T {scoreLabel(member.total)}</b></div>) ?? <span>אין member frames.</span>}</div></div></div><div className="empty-state" style={{ marginTop: 16 }}><FileChartColumn /><strong>PDF truth-backed עדיין לא הופק</strong><span>הדוח לא יסומן מוכן עד ש־BW-REP/PDF יעבוד מאותם event frames ו־recompute results.</span></div></div>;
}
