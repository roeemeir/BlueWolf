...
function exportPdfReport(server: string, from: string, to: string, events: InvestigationEvent[], weightedScore: number, totalMinutes: number) {
  const popup = window.open("", "_blank", "noopener,noreferrer");
  if (!popup) { toast.error("הדפדפן חסם את חלון ה־PDF"); return; }
  const eventHtml = events.map((event, index) => `<section class="event"><header><div><small>EVENT ${index + 1}</small><h2>${event.group}</h2><p>${event.start}–${event.end} · ${event.durationMin} דקות</p></div><strong>${event.score}</strong></header><div class="scores"><span>סנכרון <b>${event.sync}</b></span><span>נתיב <b>${event.route}</b></span></div><table><thead><tr><th>Root cause</th><th>% זמן</th><th>השפעה</th><th>תרומה</th></tr></thead><tbody>${event.rootCauses.map((cause) => `<tr><td>${cause.label}</td><td>${cause.sharePct}%</td><td>−${cause.impactPoints}</td><td>−${cause.contribution}</td></tr>`).join("")}</tbody></table></section>`).join("");
  popup.document.write(`<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><title>Blue Wolf Report</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;color:#102735;background:white}.hero{padding:22px;border-radius:22px;color:white;background:linear-gradient(135deg,#061d37,#0a6f89 60%,#20b8a6)}.hero small{opacity:.75}.hero h1{margin:4px 0;font-size:28px}.hero p{margin:0;opacity:.86}.kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:16px 0}.kpis article{padding:14px;border:1px solid #dce8ed;border-radius:14px;background:#f6fafb}.kpis span{display:block;color:#6c7f88;font-size:11px}.kpis b{font-size:24px}.map{height:180px;border:1px solid #dce8ed;border-radius:18px;background:radial-gradient(circle at 20% 50%,rgba(24,169,155,.18),transparent 27%),radial-gradient(circle at 72% 45%,rgba(76,114,232,.18),transparent 32%),#f5fafb;position:relative;overflow:hidden}.map:before{content:"SI";position:absolute;right:90px;top:58px;width:80px;height:80px;border:3px solid #18a99b;border-radius:50%;display:grid;place-items:center;color:#18a99b;font-weight:800}.map:after{content:"SO · 30°";position:absolute;left:90px;top:72px;width:180px;height:50px;border:3px solid #4c72e8;border-radius:26px;display:grid;place-items:center;color:#4c72e8;font-weight:800;transform:rotate(-8deg)}.event{margin-top:14px;padding:15px;border:1px solid #dce8ed;border-radius:16px;break-inside:avoid}.event header{display:flex;justify-content:space-between;align-items:center}.event h2{margin:3px 0}.event p{margin:0;color:#6c7f88}.event header>strong{font-size:32px;color:#0c7c82}.scores{display:flex;gap:8px;margin:10px 0}.scores span{padding:7px 10px;border-radius:999px;background:#eef6f7}table{width:100%;border-collapse:collapse;font-size:11px}th,td{padding:7px;border-bottom:1px solid #e5edef;text-align:right}th{color:#637982;background:#f6fafb}@media print{.no-print{display:none}}</style></head><body><section class="hero"><small>BLUE WOLF · RETROSPECTIVE</small><h1>זאב כחול · דוח תחקור</h1><p>${server} · ${from.replace("T"," ")}–${to.replace("T"," ")}</p></section><section class="kpis"><article><span>אירועים</span><b>${events.length}</b></article><article><span>זמן בקבוצות</span><b>${totalMinutes} דק׳</b></article><article><span>ציון משוקלל</span><b>${weightedScore}</b></article></section><div class="map"></div>${eventHtml}<script>setTimeout(()=>window.print(),350)<\/script></body></html>`);
