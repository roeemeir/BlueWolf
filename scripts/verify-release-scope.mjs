...
export function validateReleaseScope(manifest) {
  ...
  if (manifest.schemaVersion !== 'bluewolf.release-scope.v1') errors.push('unsupported release scope schemaVersion');
  ...
  for (const [index, requirement] of manifest.requirements.entries()) {
    ...
    if (requirement.userApproved !== true) errors.push(`${id || prefix} is not marked user-approved`);
    if (!VALID_IMPLEMENTATION.has(requirement.implementation)) errors.push(`${id || prefix} has invalid implementation status`);
    if (requirement.implementation !== 'yes') errors.push(`${id || prefix} blocks release: implementation=${requirement.implementation}`);
...
if (invoked) {
  ...
  try {
    ...
    if (errors.length) {
      console.error('RELEASE BLOCKED — frozen requirements are not fully implemented/evidenced:');
      for (const error of errors) console.error(`- ${error}`);
      process.exitCode = 1;
    } else {
      console.log(`PASS: release scope ${manifestPath} contains only frozen, user-approved, implemented and evidenced requirements.`);
    }
  } catch (error) {
