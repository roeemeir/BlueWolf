...
test("investigation uses Core event evidence, real ranges, synchronized score graphs and WGS84 navigation", async () => { const investigation = await readFile(path.join(root, "components/bluewolf/investigation-view.tsx"), "utf8"); assert.match(investigation, /אירוע מוגדר כרצף/); assert.match(investigation, /Core Event Archive/); assert.match(investigation, /\/api\/investigation\/events/); assert.match(investigation, /\/api\/investigation\/recompute/); assert.match(investigation, /rootCauses/); assert.match(investigation, /occurrences/); assert.match(investigation, /missingFrameCount/); assert.match(investigation, /scoredFrameCount/); assert.match(investigation, /אין progress מומצא/); assert.match(investigation, /פער בגרף/); assert.match(investigation, /type="datetime-local"/); assert.match(investigation, /טען טווח מהארכיון/); assert.match(investigation, /SQLite/); assert.match(investigation, /Group \+ vehicle scores/); assert.match(investigation, /type="range"/); assert.match(investigation, /סליידר זמן אחיד לתחקור/); assert.match(investigation, /cursorPoint\.members/); assert.match(investigation, /navigation evidence/); assert.match(investigation, /WGS84 אמיתי/); assert.match(investigation, /result\.points\[index\]\?\.navigation/); assert.match(investigation, /headingDeg/); assert.match(investigation, /BW-REP-010.*עדיין פתוח/); assert.doesNotMatch(investigation, /getServerScenario/); assert.doesNotMatch(investigation, /buildEvents/); assert.doesNotMatch(investigation, /window\.print/); });

test("uses discrete threshold grids instead of free numeric threshold inputs", async () => { const developer = await readFile(path.join(root, "components/bluewolf/developer-view.tsx"), "utf8"); assert.match(developer, /v04-threshold-grid/); assert.doesNotMatch(developer, /type="number"/); assert.match(developer, /SI_POSITION_ANGLES/); assert.match(developer, /SO_RELATION_LABELS/); });
