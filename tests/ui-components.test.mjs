...
test("emits the Blue Wolf dashboard layout and v0.4 map styles", async () => { const css = await readCssTree(path.join(root, "dist")); assert.match(css, /\.app-shell/); assert.match(css, /\.map-stage/); assert.match(css, /\.timeline-svg/); assert.match(css, /\.developer-workspace/); assert.match(css, /\.v04-live-map/); assert.match(css, /\.v04-route-bank-map/); assert.match(css, /\.v04-root-causes/); assert.match(css, /\.dark\s*\{/); });
