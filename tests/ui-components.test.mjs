...
test("limits sequential SI relations to 45, 90 and 120 degrees", async () => { const { SI_ALLOWED_PAIR_ANGLES, generateSiAngleSets } = await vite.ssrLoadModule("/lib/bluewolf.ts"); assert.deepEqual([...SI_ALLOWED_PAIR_ANGLES], [45, 90, 120]); const vehicleCount = 5; const patterns = generateSiAngleSets(vehicleCount); assert.ok(patterns.length >= 3); assert.ok(patterns.length <= 6); assert.ok(patterns.every((values) => values.length === vehicleCount - 1)); assert.ok(patterns.every((values) => values.every((value) => SI_ALLOWED_PAIR_ANGLES.includes(value)))); });
...
test("investigation is based on group continuity and aggregated root causes", async () => { const investigation = await readFile(path.join(root, "components/bluewolf/investigation-view.tsx"), "utf8"); assert.match(investigation, /אירוע מוגדר כרצף/); assert.match(investigation, /sharePct/); assert.match(investigation, /impactPoints/); assert.match(investigation, /contribution/); assert.match(investigation, /EventOverviewMap/); assert.match(investigation, /draftEdits/); assert.match(investigation, /updateDraft/); assert.doesNotMatch(investigation, /useEffect/); assert.doesNotMatch(investigation, /updateEdit/); });
