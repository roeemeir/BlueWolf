...
test("investigation uses Core event evidence and keeps missing frames explicit", async () => { const investigation = await readFile(path.join(root, "components/bluewolf/investigation-view.tsx"), "utf8"); assert.match(investigation, /אירוע מוגדר כרצף/); assert.match(investigation, /Core Event Archive/); assert.match(investigation, /\/api\/investigation\/events/); assert.match(investigation, /\/api\/investigation\/recompute/); assert.match(investigation, /rootCauses/); assert.match(investigation, /occurrences/); assert.match(investigation, /missingFrameCount/); assert.match(investigation, /scoredFrameCount/); assert.match(investigation, /אין progress מומצא/); assert.match(investigation, /פער בגרף/); assert.doesNotMatch(investigation, /getServerScenario/); assert.doesNotMatch(investigation, /buildEvents/); assert.doesNotMatch(investigation, /window\.print/); });
