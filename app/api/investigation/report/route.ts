...
type ReportOverride = { eventId: string; templateId?: string | null; arena?: string | null; note?: string | null };
...
function optionalIso(value: unknown, name: string) {
  ...
  if (!Number.isFinite(parsed.getTime())) throw new Error(`${name} must be an ISO-8601 timestamp`);
...
function parseRequest(value: unknown): ReportRequest {
  ...
  const overrides = rawOverrides.map((item, index) => {
    ...
    const templateId = override.templateId == null ? null : String(override.templateId).trim();
    const arena = override.arena == null ? null : String(override.arena).trim();
    const note = override.note == null ? null : String(override.note).trim();
...
async function coreJson(baseUrl: string, token: string | undefined, path: string, init?: RequestInit, timeoutMs = 90_000) {
  ...
  try {
    ...
    if (!response.ok) {
      const detail = payload && typeof payload === "object" && "error" in payload ? String((payload as { error: unknown }).error) : `Python Core returned ${response.status}`;
    ...
    if (error instanceof Error && error.name === "AbortError") throw new Error("Python Core report request timed out");
    throw error;
  } finally {
...
async function recomputeEvent(
  ...
  const payload = await coreJson(baseUrl, token, "/v1/investigation/recompute", {
    method: "POST",
    body: JSON.stringify({ eventId, templateId, scenarioId: `pdf-report:${generatedAt}:${eventId}` }),
  }, 120_000);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ status: "error", error: error instanceof Error ? error.message : "invalid report request" }, { status: 400 });
  }

  const query = new URLSearchParams({ serverId: String(parsed.serverId) });
  ...
  try {
    ...
    const prepared = listing.events.map((event) => {
      const override = overrideByEvent.get(event.eventId);
      const templateId = override?.templateId || event.activeTemplateId;
      return { event, override, templateId };
    });
    const missing = prepared.filter((item) => !item.templateId).map((item) => item.event.eventId);
    if (missing.length) {
      return Response.json({
        status: "unavailable",
        error: "report cannot choose a template without archived or explicit provenance",
        missingTemplateEvents: missing,
      }, { status: 422 });
    }
    ...
    const reportEvents: InvestigationPdfEvent[] = await mapWithConcurrency(prepared, 3, async ({ event, override, templateId }) => {
      ...
      if (result.eventId !== event.eventId || result.serverId !== parsed.serverId || result.templateId !== templateId) {
        throw new Error(`recompute provenance mismatch for ${event.eventId}`);
      }
      return { result, arena: override?.arena ?? null, note: override?.note ?? null };
    });

    const codeVersions = new Set(reportEvents.map((item) => item.result.codeVersion));
    const configVersions = new Set(reportEvents.map((item) => item.result.configVersion));
    if (codeVersions.size !== 1 || configVersions.size !== 1) {
    ...
    const mappedStatus = status === 404 || status === 422 ? status : status === 401 ? 502 : status >= 500 ? 503 : 502;
